...
clear all
n=100; k=2; nter=50+1; %nter is the encoding time step t
...
for jj=1:50  % generate 50 random pair
   [v, u] = generateOrthogonalVectors(1); % generate vector u and v othogonal to each other
    ...
    while ii < nter
        ...
        dis=(sum((u-v).^2)); %compute squared Euc distance 
...
ylabel('$d(w_t,w''_t)^2$', 'Interpreter', 'latex');

ylim([0, 4]); xlim([0.85, nter-1]);
